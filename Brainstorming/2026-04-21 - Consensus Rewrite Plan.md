...
- [~] **Phase 1d**: *(1d.1 done)*
  ...
  - [x] **1d.2**: Verbatim/clean toggle in the review header. Shipped without touching `LLMReconcileService` by pairing the LLM-reconciled output (clean) with Engine A's pre-reconciliation segments (closer-to-audio, preserves disfluencies) via time-overlap alignment. Stored as `StylePair` on the `.deep` pass. Toggle is a capsule-style two-chip control that persists the choice on `project.settings.transcriptStyle`. Exports respect the current style via a `renderActivePass` style-substitution step. Full dual-LLM-output (single-prompt, both from the model) stays an option for later if the approximation proves weak on benchmark.
  - [x] **1d.3**: Alternatives in the uncertainty popover, sourced from Engine A and Engine B's pre-reconciliation segments via time-overlap (no LLM prompt change required). `TranscriptPass` gains `alternativesByIndex: [Int: [TurnAlternative]]` keyed by segment index, populated by `DeepPassRunner` at pass time. Popover shows an "ALTERNATIVES" section with up to two click-to-apply buttons — each with an engine source label, ⌘1 / ⌘2 shortcut hint, and the alternative text (3-line preview). Clicking replaces `segments[i].text` + `styles.cleanText[i]` and auto-marks the turn resolved. Manual free-form override still lives in the existing Manual Editor.
