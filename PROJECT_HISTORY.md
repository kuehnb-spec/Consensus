...
Long-form validation completed after the commit: the 36-minute Maralan recording produced 150 segments with zero non-monotonic timestamps and full coverage to the final second, at 0.21x real-time (about 4.8x faster than realtime) on the M2 Max — comfortably inside the spec's faster-than-realtime goal.

## 2026-07-25 (later) — CLI portability: config-resolved paths, doctor, install package

Brant asked for the new build to be published for download onto his Mac Studio. Investigation found it would have been a download that could not run: the sidecar paths were hardcoded to this development checkout, so any other machine would fail at inference with nothing actionable. Reported that before publishing anything.
Added `ConsensusConfig`, which resolves the Python interpreter, sidecar script, and model through env vars → `~/.consensus/config.toml` → conventional `~/.consensus/` layout → Hugging Face cache → repository discovery (walking up from the binary). Removed the hardcoded developer path from shipping code entirely and gave this machine a real config file instead, which also made a clean-machine test possible.
Added `consensus doctor`, which reports each dependency, where it resolved from, and the exact remedy when missing; `transcribe` now preflights the same check and fails fast with a legible message instead of dying inside the model load.
Built a distribution package: `Packaging/install.sh` (binary, sidecar, venv from pinned requirements, model download, config write, verification), `build-release.sh`, and an install README. Output is an 11–12 MB tarball containing code only — the build script deliberately stages named files rather than sweeping a directory, because this repository holds real client recordings.
Verified: env/config/repo resolution each selected correctly; a clean environment reports all three dependencies missing and exits 3; transcribe preflight fails fast; the installer built a working venv in a scratch location and that freshly installed environment transcribed real audio correctly. Untested: the Hugging Face cache branch (this machine's model lives in the dev rig, and its HF cache holds only a refs stub) and the ~5.3 GB model download itself.
Decided against committing binaries to the personal-website repository — GitHub rejects files over 100 MB and the site deploys from git; GitHub Releases with a link from the project page is the right home.

## 2026-07-30 — Mac Studio cleanup, full-build link fix, Quick Take becomes a real pushbutton

First session on the Mac Studio after the move from the MacBook Pro. Cleared the migration debris: removed the committed pre-rename `BDK Transcribo.app` bundle (19 MB of binaries in git) and four stray swiftc droppings from a direct compiler invocation, hardened `.gitignore` against both, archived the untracked 859-line three-column ReconciliationView experiment (compiled into every build, zero call sites) beside the other legacy views, and wiped the stale 1.9 GB `.build` cache for a from-scratch rebuild on the Studio's Swift 6.3.3 toolchain.
The fresh full build exposed a real bug the MacBook had been masking: the `Consensus` (GUI) and `consensus` (CLI) executable products stage into case-colliding `.product` directories, so a full `swift build` linking both at once corrupts one link (`_ConsensusCommandLine_main` missing). The July 25 fix had renamed the target but not the product. Renamed the GUI product to `ConsensusApp` — the CLI keeps its deployed `consensus` name — and `build-app.sh` now stages the renamed binary into the bundle as `Consensus`.
Started Goal B's three-rooms build-out with Quick Take: it no longer blocks on the speaker-naming stage (high-confidence intro-scan names auto-apply; everyone else stays "Speaker N" with inline rename on the result view), the deep-pass failure path now degrades silently to Standard in Quick Take per the June 30 no-scary-alerts decision, and the workstation progress/review screens are replaced by calm appliance-grade equivalents in a new `QuickTakeViews.swift` — a quiet progress screen with human-language status lines and a readable result document with a single Export action bar plus a "Fine-tune in Deep Read" escape hatch.
The progress element debuts the visual-identity motif for B3: `ConvergenceMark`, a Canvas-drawn animation of three strands converging into one line — consensus being forged — which will scale into the wordmark and app icon. Also refreshed the README's dead build instructions (it still documented the pre-split `swift run Transcribo` smoke path). Runtime click-through of the new Quick Take flow on real audio is the next session's first move.

## 2026-09-23 — Consensus Next kickoff: Proving Ground, Nemotron 3 measured, UI direction

Brant asked for four things: use NVIDIA's new diarizer, rethink the UI and file handling, find test material that needs no hand-editing (he suggested famous film scenes), and reopen pipeline experiments. Chose to measure before recommending.
Built the Proving Ground (`Scripts/proving-ground/`): an Oyez fetcher that cuts the 10-minute window with the most speaker changes from public-domain Supreme Court arguments, with speaker-named, time-aligned reference transcripts; an engine runner; a words-from-one-engine, speakers-from-another combiner; a meeteval-based scorer (content WER, cpWER, DER); and a reference audit that uses two engines as witnesses and lists turns where they agree with each other but not the reference. Rejected movies as the corpus (copyright, cleaned and unaligned fan transcripts, music under dialogue, acted speech). The 2020 term's telephone arguments supply legal phone-call audio.
The first corpus is 15 clips (149 min, 6–10 speakers). NVIDIA's Nemotron 3 Diarization (released today, 8 speakers, commercial license) ran on-device through a freshly built FluidAudio 0.17.1. Results: the current VibeVoice CLI leads at 5.5% content WER, 8.2% cpWER, 3.0% DER. VibeVoice words with Nemotron speakers scored 12.4% cpWER, but won 5 of 15 clips (Weyerhaeuser DER 10.3% → 1.3%). Decision: Nemotron becomes a second speaker witness and the fallback past 60 minutes, not a replacement. Parakeet v3 is 40× faster than VibeVoice at 7.6% WER, with differently placed errors ("ripeness" vs VibeVoice's "rightness").
The audit showed the official transcripts are clean verbatim, so a shared normalizer now collapses stutters and repeats on both sides. That cut false flags, leaving 29 of 1,176 turns (2.5%) for Brant to spot-check. It also found that VibeVoice emits a copied 0.95 as every word's confidence, which breaks the no-fabricated-confidence guardrail.
Found and fixed a launch crash in the installed app: SwiftPM's `Bundle.module` only checks the `.app` root and the deleted `.build` path. `build-app.sh` now links resource bundles at the bundle root, verified on the installed app. A rebuild is blocked because the Xcode 27 update removed the Metal Toolchain; the Studio also has only 31 GB free.
Wrote the proposal with an interactive mockup (`Brainstorming/2026-09-next/consensus-next.html`): one local library with Matters and an Inbox, `.consensus` packages shared by GUI and CLI, passes as version history, and modes as lenses (Read / Review / Studio) separate from accuracy depth. The identity is built from the legal record: page:line gutter, highlighter disputes, and a convergence strip showing engines agreeing or splaying apart.

## 2026-09-25 — Build fixed on Xcode 27, Nemotron witness test, identity approved, motion prototype

Brant installed the Metal Toolchain. The rebuild then failed because the Xcode 27 toolchain's SwiftPM moved products to `.build/out/Products/<Config>`. `build-app.sh` now asks `swift build --show-bin-path`. A fresh release bundle launched with `.build` moved aside, confirming the bundle-root resource links, and is installed at `/Applications/Consensus.app`.
Brant pushed back on the Nemotron verdict: even if it loses head-to-head, doesn't the Consensus theory say a second witness helps unless it makes the same mistakes? Measured it word by word. VibeVoice credits 2.15% of words to the wrong speaker. Nemotron disagrees on 4.6% of words and catches 37% of those errors, and perfect resolution of its flags would take cpWER from 8.18% to 7.45%. But Nemotron is right in only 8–13% of its disputes on this 6–10-speaker corpus. Its errors correlate with community-1's, so 2-of-3 voting breaks more than it fixes (10.48%), and span-level CAM++ voice matching also lost at every margin (best 8.28%). The theory holds in principle; no automatic resolver captures it yet. The next test is 2–3-speaker audio.
A trap worth remembering: word-level truth built from Oyez timestamps made a perfect oracle look worse on cpWER while improving DER against the same drifted clock. Truth now comes from text alignment.
Brant approved the legal-record identity and asked for motion. Built a motion reel into the proposal: threads converge (signature), wet ink dries, highlighter swipe and erase, re-listen loupe, speaker lanes, and a Verified seal. The rule is that every motion reports a real event. AGENTS.md's design rules were rewritten to match, replacing the indigo/glass rules.

## 2026-09-25 (later) — Release pipeline: signed app, public download staged

Brant asked to publish the new build on brantkuehn.com for his other Macs and for other people. Built `Packaging/release.sh`: a Developer ID-signed, hardened-runtime app in a signed DMG (90 MB) plus the signed CLI and engine installer (10 MB), with notarization gated on a stored notarytool profile.
Getting there surfaced four problems that would have shipped broken. (1) The bundle-root resource links from the Sept 23 crash fix made the app unsignable. The Xcode 27 SwiftPM accessor already looks in `Contents/Resources`, so the links are gone; verified launching with `.build` moved aside. (2) The app hard-coded VibeVoice, so on any Mac without the engine it would error instead of transcribing; it now falls back to Parakeet. (3) The engine installer used whatever `python3` it found, but macOS ships 3.9 and the pinned wheels need 3.11+; the installer now finds a suitable interpreter or says exactly what to install. (4) The website's "Download the CLI" link pointed at a GitHub release that never existed.
Both engines were smoke-tested inside the signed, hardened app. App and CLI are now 2.1.0. A draft GitHub release (v2.1.0, both assets) and the website download section (branch `consensus-2.1-download`, not pushed) wait on Brant's notarization credentials and his OK on the copy.
